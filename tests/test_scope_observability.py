...
def test_scope_observability_logs_and_counts_project_fallback(caplog) -> None:
    ...
    assert int(payload["fallback_counts_by_endpoint"].get("/project") or 0) >= 1
    ...
    assert any(
        "scope.observability.fallback" in record.getMessage()
        for record in caplog.records
    )


def test_scope_observability_counts_graph_resolve_reference_fallback() -> None:
    backend_main._reset_scope_observability_for_tests()
    client = TestClient(backend_main.app)
    ...
    assert counters.status_code == 200
    payload = counters.json()
    assert int(payload["fallback_counts_by_endpoint"].get("/graph/resolve-references") or 0) >= 1
